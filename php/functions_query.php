...
function get_capacities_poke($mysqli,$id){
    return readDB($mysqli, "SELECT capacite.libelle_capacite,capacite.pp_capacite,capacite.puissance_capacite,capacite.precision_capacite,type.libelle as libelle_type FROM capacite JOIN type ON capacite.id_type = type.id_type JOIN lance ON capacite.id_capacite = lance.id_capacite WHERE lance.id_pokemon = $id ");
}
...
function dresseur_exists($mysqli,$login,$password){
    ...
    return readDB($mysqli, "SELECT * FROM dresseur WHERE nom_dresseur = '$login' AND mdp_dresseur = '$password' ");
